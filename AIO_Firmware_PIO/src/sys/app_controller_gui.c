...
void app_control_gui_init(void)
{
    ...
    // To eliminate the partial white screen problem at startup, add the following layer (can be considered for deletion and improvement)
...
void display_app_scr_init(const void *src_img_path, const char *app_name)
{
    ...
    if (act_obj == app_scr)
    {
        // Prevent the screen from not being drawn and updated without other actions when some apps that do not use lvgl exit
    ...
    // lv_label_set_recolor(pre_app_name, true); // Enable text recoloring first
...
{
    // force determines whether to force refresh the page, true for force refresh
    if (true == force)
    {
    ...
    {
        // 120 is half the screen size, the application icon is 128, half is exactly 64
        now_start_x = -120 - 64;
        now_end_x = 0;
        old_start_x = 0;
        old_end_x = 120 + 64;
    ...
    // lv_label_set_recolor(now_app_name, true); // Enable text recoloring first
    ...
    lv_anim_set_path_cb(&now_app, lv_anim_path_linear); // Set the path of the animation
    ...
    lv_anim_set_path_cb(&pre_app, lv_anim_path_linear); // Set the path of the animation

    lv_anim_start(&now_app);
    lv_anim_start(&pre_app);
    ANIEND_WAIT
    lv_task_handler(); // Eliminate the problem of "stuttering" after ANIEND_WAIT is executed

    lv_obj_del(pre_app_image); // Delete the original image
